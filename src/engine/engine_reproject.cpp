 ...
 *
 * For each output chunk the engine:
 *  1. Computes a 32×32 coarse control-point grid.
 *  2. Derives a source bounding box (canvas) from the grid.
 *  3. Reads source tiles/strips into the canvas.
 *  4. Uploads the canvas to CUDA texture arrays.
 *  5. Launches the GPU warp kernel.
 *  6. Launches the algebra and optional mask kernels.
 *  7. Delivers results via PipelineCtx callbacks.
 */
#include "engine.h"
...
// ─── Local GDAL dataset pool (mirrors the one in engine.cpp) ─────────────────
...
void run_engine_reproject(const std::string& input_file, PipelineCtx& ctx, bool verbose) {
    ...
    // Output chunks are aligned to the output tile height.
    ...
    // Sample a small area at the centre of the output to estimate how many
    // source pixels correspond to one destination pixel in each direction.
    double h_scale = 1.0, v_scale = 1.0;
    {
        ...
        double dx = std::sqrt(std::pow(probe_x[1] - probe_x[0], 2)
                            + std::pow(probe_y[1] - probe_y[0], 2)) / 100.0;
        double dy = std::sqrt(std::pow(probe_x[2] - probe_x[0], 2)
                            + std::pow(probe_y[2] - probe_y[0], 2)) / 100.0;

        if (dx > 0 && dx < 1000) { h_scale = dx; }
        if (dy > 0 && dy < 1000) { v_scale = dy; }
        probe.destroy();
    }

    // ── Canvas size bounds ─────────────────────────────────────────────────
    // Add generous margin so the canvas covers slight mismatches at tile edges.
    int max_canvas_width  = std::min(src_info.width,
        static_cast<int>(out_info.width  * h_scale * 1.5 + src_info.tile_width  * 2 + 128));
    int max_canvas_height = std::min(src_info.height,
        static_cast<int>(chunk_height    * v_scale * 1.5 + src_info.tile_height * 2 + 128));

    size_t canvas_bytes_per_thread = static_cast<size_t>(max_canvas_width)
                                   * max_canvas_height * num_bands * sizeof(float);
    size_t output_band_bytes       = static_cast<size_t>(out_info.width) * chunk_height * sizeof(float);
    ...
    if (canvas_bytes_per_thread > 0) {
        warp_threads = std::min(warp_threads,
            static_cast<int>(std::max(size_t{1}, vram_budget / canvas_bytes_per_thread)));
    }
    size_t pinned_per_thread = output_band_bytes * num_bands + canvas_bytes_per_thread;
    if (pinned_per_thread > 0) {
        warp_threads = std::min(warp_threads,
            static_cast<int>(std::max(size_t{1}, g_pinned_budget / pinned_per_thread)));
    }
    ...
    std::vector<ThreadBufs>       thread_pool(warp_threads);
    std::vector<WarpTransformer>  transformers(warp_threads);

    for (int t = 0; t < warp_threads; ++t) {
        thread_pool[t].alloc(output_band_bytes, num_bands,
                             ctx.has_clip_mask ? chunk_height : 0);
        thread_pool[t].alloc_warp(max_canvas_width, max_canvas_height, max_dst_pixels,
                                  num_bands, ctx.reproject_params.resampling);
    ...
    if (!is_s3) {
        for (int t = 0; t < warp_threads; ++t) {
            dataset_pool.datasets[t] = static_cast<GDALDataset*>(
    ...
    size_t tiles_across = ((size_t)src_info.width  + src_info.tile_width  - 1) / src_info.tile_width;
    size_t tiles_down   = ((size_t)src_info.height + src_info.tile_height - 1) / src_info.tile_height;
    ...
    for (int chunk = 0; chunk < num_chunks; ++chunk) {
        ...
        // Convert double coarse grid to float, relative to canvas top-left.
        ...
        memset(buf.h_src_canvas, 0,
               static_cast<size_t>(canvas_bbox.w) * canvas_bbox.h * num_bands * sizeof(float));

        if (!is_s3) {
            for (int b = 0; b < num_bands; ++b) {
                dataset_pool.bands[thread_id][b]->RasterIO(
                    GF_Read, canvas_bbox.x0, canvas_bbox.y0,
                    canvas_bbox.w, canvas_bbox.h,
                    buf.h_src_bands[b], canvas_bbox.w, canvas_bbox.h,
                    GDT_Float32, 0, 0);
            }
                    ...
                    if (is_pixel_interleaved) {
                        ...
                        if (ti < src_info.tile_offsets.size()) {
                            fetch_jobs.push_back({ti,
                                src_info.tile_offsets[ti], src_info.tile_lengths[ti], {}, 0});
                        }
                    } else {
                        for (int b = 0; b < num_bands; ++b) {
                            size_t ti = static_cast<size_t>(band_slots[b]) * tiles_across * tiles_down
            ...
            s3_fetch_tiles(s3_location, src_info.tile_offsets, src_info.tile_lengths, fetch_jobs);
            ...
            for (auto& job : fetch_jobs) {
                ...
                auto float_tile = decompress_tile_to_float(
                    ...
                    src_info.tile_width, src_info.tile_height, block_spp,
                    src_info.compression, src_info.predictor, src_info.data_type);
                ...
                if (!is_pixel_interleaved) {
                    ...
                    tr     = static_cast<int>((ti % (tiles_across * tiles_down)) / tiles_across);
                    tc     = static_cast<int>(ti % tiles_across);
                    band_plane = b;
                }

                fill_canvas_from_tile(
                    float_tile.data(), tr * src_info.tile_height, tc * src_info.tile_width,
                    src_info.tile_width, src_info.tile_height, block_spp,
                    is_pixel_interleaved, band_slots, buf.h_src_bands, canvas_bbox, band_plane);
            }
                ...
                for (int sr = sr0; sr <= sr1; ++sr) {
                    ...
                    if (si < src_info.strip_offsets.size()) {
                        fetch_jobs.push_back({si,
                            src_info.strip_offsets[si], src_info.strip_lengths[si], {}, 0});
                    }
                }
                if (is_pixel_interleaved) { break; }
            }

            s3_fetch_tiles(s3_location, src_info.strip_offsets, src_info.strip_lengths, fetch_jobs);
            ...
            for (auto& job : fetch_jobs) {
                ...
                auto float_strip = decompress_tile_to_float(
                    job.data.data(), job.data.size(),
                    src_info.width, strip_num_rows, block_spp,
                    src_info.compression, src_info.predictor, src_info.data_type);

                fill_canvas_from_strip(
                    float_strip.data(), strip_first_row, strip_num_rows,
                    src_info.width, block_spp, is_pixel_interleaved,
                    band_slots, buf.h_src_bands, canvas_bbox, -1);
            }
        }

        // ── Phase 4: Upload coarse grid and canvas textures to GPU ────────
        CUDA_CHECK(cudaMemcpyAsync(buf.d_coarse_x, coarse_x_f,
                                   coarse_grid_bytes, cudaMemcpyHostToDevice, buf.cuda_stream));
        CUDA_CHECK(cudaMemcpyAsync(buf.d_coarse_y, coarse_y_f,
                                   coarse_grid_bytes, cudaMemcpyHostToDevice, buf.cuda_stream));
        buf.bind_src_canvas(num_bands, canvas_bbox.w, canvas_bbox.h, buf.cuda_stream);
        ...
        launch_warp_kernel(
            ...
            static_cast<size_t>(out_info.width) * chunk_height,
            buf.cuda_stream);
        ...
        launch_raster_algebra(
            ...
            const_cast<const float* const*>(buf.d_warp_band_ptrs),
            buf.d_output,
            dst_pixels,
            buf.cuda_stream);
        ...
        if (ctx.has_clip_mask && buf.d_clip_spans) {
            ...
            launch_apply_mask(buf.d_output, dst_pixels,
                              out_info.width, buf.d_clip_spans, buf.cuda_stream);
        }
        ...
        if (ctx.output_band) {
            #pragma omp critical(gdal_write)
            {
                ctx.output_band->RasterIO(
                    GF_Write, 0, chunk_y0, out_info.width, cur_height,
                    buf.h_output, out_info.width, cur_height,
                    GDT_Float32, 0, 0);
            }
        }
        if (ctx.queue_callback)  { ctx.queue_callback (out_info.width, cur_height, buf.h_output, chunk_y0); }
        if (ctx.result_callback) { ctx.result_callback(out_info.width, cur_height, buf.h_output, chunk_y0); }
    ...
    for (auto& warp : transformers)  { warp.destroy(); }
